...
final class SettingsPanel implements ChangeListener, ItemListener
{
  ...
  SettingsPanel(final UI pUI)
  {
    ...
    _checkBoxes = new HashMap<JCheckBox, String>();
    ...
    addCheckBox("panel.settings.highlightLastMove", HIGHLIGHT_LAST_MOVE_PROPERTY);
    ...
    modifTaille.add(new JLabel(getI18NString("panel.settings.cellsSize")), BorderLayout.WEST);
    _cellSize = new JSlider(1, 5);
    _cellSize.setValue((Integer.parseInt(_ui.getProperty(CELLS_SIZE_PROPERTY)) - 40) / 8);
    _cellSize.setMajorTickSpacing(1);
    _cellSize.setSnapToTicks(true);
    _cellSize.setPreferredSize(new Dimension(50, _cellSize.getPreferredSize().height));
  ...
  private void addCheckBox(final String pIdLabel, final String pPropriete)
  {
    ...
    final JCheckBox cb = new JCheckBox(getI18NString(pIdLabel), _ui.getBooleanProperty(pPropriete));
  ...
  public void stateChanged(final ChangeEvent pEvent)
  {
    ...
    if (pEvent.getSource() == _cellSize)
    {
      ...
      {
        _ui.setProperty(CELLS_SIZE_PROPERTY, Integer.toString(_cellSize.getValue() * 8 + 40));
      }
    }
    else
    {
